...
@DynamicInsert // 필드에 null이 들어가 있다면 INSERT 쿼리에서 해당 필드를 제외한다
@DynamicUpdate // 필드에 null이 들어가 있다면 UPDATE 쿼리에서 해당 필드를 제외한다
public class User extends Period {
    ...
    @Embedded
    @AttributeOverrides({ // not null 제약 조건을 걸기 위해 사용했다. (또는 Address 타입의 필드에 직접 설정)
            @AttributeOverride(name = "address", column = @Column(nullable = false)),
            @AttributeOverride(name = "detail", column = @Column(nullable = false)),
            @AttributeOverride(name = "zipcode", column = @Column(nullable = false))
    })
    private Address address;

    @Enumerated(EnumType.STRING)
    @ColumnDefault("'N'") // 컬럼에 디폴트 값을 설정
