...
public class OptionalTest {
    ...
    void exam01() {
        /**
         * 개발을 할 때 가장 많이 발생하는 예외 중 하나가 NPE
         * 옵셔널 클래스는 NPE를 방어하기 위해 사용
         * 참조변수에 들어있는 값이 null인경우 접근 연산자(.)을 이용하여 멤버에 접근하면 NPE가 발생할 수 있다
         * if문을 이용하여 null체크를 하게되면 경우의 수가 너무 많아서 개발이 피곤해진다
         * Optional타입은 null 체크를 보다 간결하고 안전하게 처리할 수 있다.
         */
...
//      옵셔널 객체에 저장된 값을 반환한다. null인 경우 [대체 값]이 반환된다.
        Object o2 = o.orElse(3L);
        System.out.println(o2);

//      3. orElseGet(람다식)
//      옵셔널 객체가 저장된 값을 반환한다. null인 경우 [람다식]이 실행된다.
        o.orElseGet(() -> 3L);
...
//      null이 아닌 경우에 실행할 람다식과 null인 경우에- 실행할 람다식
        o.ifPresentOrElse(
