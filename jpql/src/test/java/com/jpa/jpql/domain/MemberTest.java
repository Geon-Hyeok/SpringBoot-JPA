...
//      쿼리 실행
    ...
    @Test
    void jpql2() {
//      createQuery()의 결과는 TypedQuery에 만들어진다.
...
//      id만 가져오고 싶은 경우에 select id를 명시하고 id의ㅡ 타입 Long을 반환타입으로 설정
//      select절에 작성한 조회ㅗ 대상을 프로젝션 이라고한다.
...
//      타입을 설정하지 않거나 그럴 수 없는 경우 Query타입이 반환된다.
...
//          getResultList()는 조회결과가 0, 1, 2 이상이여도 상관 없으나,
//          getSingleResult()는 조회결과가 반드시 1개여야 한다. 0건이거나 2건 이상이면 예외가 발생된다.        
        Board singleList = entityManager.createQuery(query1, Board.class)
...
//      직접 on절 사용하기
//      식별자 확인 조건은 유지되고, on절에 작성한 조건이 추가된다.
        String query3 = "SELECT b FROM Board b JOIN b.member m ON m.name = b.content";
        entityManager.createQuery(query3, Board.class)
                .getResultList();
    }
...
//      일반적인 join은 쿼리가 실행되는 순간 모든 데이터를 가져와 영속성 컨텍스트에 엔티티를 등록하게 된다.
//      지연로딩은 해당 객체를 사용하는 시점에 조회하여 성능을 향상시킨다.
...
//      Q타입 생성하는 방법2
        ...
        List<Member> memberList = queryFactory.select(member2)
                .from(member2)
...
//      페이징 처리
        queryFactory.select(member2)
                ...
                .orderBy(member2.id.desc())
                .offset(0).limit(10) // offset() : 몇 번째 결과부터 가져올지 설정, limit() : 몇개 가져올지 설정
                .fetch();
    }
...
//      조회 대상(프로젝션)이 1개인 경우
        List<String> names = queryFactory.select(member.name).from(member)
                .where(member.address.addressDetail.eq("100호"))
                .fetch();
